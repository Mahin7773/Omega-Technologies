...
if (isset($_POST["add_to_cart"])) {
    if (isset($_SESSION["shopping_cart"])) {
        ...
        if (!in_array($_GET["id"], $item_array_id)) {
            ...
            echo '<script>window.location="cooler.php"</script>';
        }
    } else {
        $item_array = array(
            'item_id' => $_GET["id"],
...
<body>
    ...
    <div class="grid">
        <?php
        $query = "select * from products where type='cooler' order by id asc";
        $result = mysqli_query($connection, $query);
        if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_array($result)) {
        ?>
                <div class="card">
                    <form method="post" action="cooler.php?action=add&id=<?php echo $row["id"]; ?>">
                        <div>
